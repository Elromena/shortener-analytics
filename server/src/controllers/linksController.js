  ...
  try {
    ...
    if (search) {
      ...
      whereConditions.push(
        `(title ILIKE $${paramCount} OR short_code ILIKE $${paramCount})`,
      );
      params.push(`%${search}%`);
    }
...
export const createLink = async (req, res) => {
  try {
    ...
    if (!brand_id || !original_url || !title) {
      return res
        .status(400)
        .json({ error: "Brand ID, URL, and title are required" });
    }
...
export const archiveLinks = async (req, res) => {
  try {
    ...
    if (verifyResult.rows.length !== linkIds.length) {
      return res
        .status(403)
        .json({ error: "Unauthorized to archive some links" });
    }

    await query(`UPDATE links SET status = 'archived' WHERE id = ANY($1)`, [
      linkIds,
    ]);
...
export const getPerformanceData = async (req, res) => {
  try {
    ...
    let whereClause = "l.brand_id = $1";
    let queryParams = [brandId];
    let paramIndex = 2;

    if (linkId) {
      // If linkId is provided, verify it belongs to the brand
      const linkResult = await query(
        "SELECT id FROM links WHERE id = $1 AND brand_id = $2",
        [linkId, brandId],
      );
      ...
      if (linkResult.rows.length === 0) {
        return res
          .status(404)
          .json({ error: "Link not found or does not belong to this brand" });
      }

      whereClause += " AND l.id = $2";
      queryParams.push(linkId);
      paramIndex = 3;
    }
    ...
    const clicksResult = await query(
      `SELECT c.clicked_at, l.platform, l.category
       FROM clicks c
       INNER JOIN links l ON c.link_id = l.id
       WHERE ${whereClause} AND c.clicked_at >= NOW() - INTERVAL '${parseInt(dateRange)} days'
       ORDER BY c.clicked_at`,
      queryParams,
    );
    ...
    for (let i = parseInt(dateRange) - 1; i >= 0; i--) {
      ...
      if (metricsObj.total) point.total = 0;
      if (metricsObj.byPlatform) {
        ["Twitter", "LinkedIn", "Facebook", "Instagram"].forEach(
          (p) => (point[p] = 0),
        );
      }
      if (metricsObj.byCategory) {
    ...
    res.json({
      ...
      metadata: {
        brandId,
        ...(linkId && { linkId }), // Include linkId in metadata if it was used
      },
    });
  } catch (error) {
    console.error("Get performance data error:", error);
